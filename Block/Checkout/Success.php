    ...
    public function getOrderDetails()
    {
        ...
        foreach ($items as $item) {
            if ($item->getData('product_type') == 'configurable') {
                $orderItemConfigurable[$item->getData('item_id')] = str_replace(",", "", number_format_defualt_to_zero($item->getData('price'), 2));
            }
        ...
        foreach ($items as $item) {
            ...
            if ($detail) {
                ...
                elseif (isset($orderItemGrouped[$detail['product_id']])){
                    $price = str_replace(",", "", number_format_defualt_to_zero($orderItemGrouped[$detail['product_id']], 2));
                    ...
                    $price = str_replace(",", "", number_format_defualt_to_zero($item->getData('price'), 2));
                }
